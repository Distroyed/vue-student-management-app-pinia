...
export default {
  ...
  created() {
    let apiURL = `http://localhost:3001/grade/${this.$route.params.id}`;

    axios.get(apiURL).then((res) => {
      this.grade = res.data;
    });

    let apiURL1 = `http://localhost:3001/grade/selections`;
    ...
    axios.get(apiURL).then((res) => {
      ...
      axios.get(`http://localhost:3001/grade/selections`).then((res) => {
        this.Selection = res.data;
      });
    });
  },
