...
// @ is an alias to /src
    ...
    export default {
        ...
        created() {
            let apiURL = `http://localhost:3001/student-grade/${this.$route.params.id}`;
            ...
            let apiURL1 = `http://localhost:3001/students/${this.$route.params.id}`;
        ...
        methods: {
            ...
            deleteGrade(id){
                let apiURL = `http://localhost:3001/grade/${id}`;
            ...
            handleSubmitForm() {
                let apiURL = 'http://localhost:3001/grade';
                
                axios.post(apiURL, this.grade).then(() => {
                  //reaload (we should actually use a component with key here)
                  this.$router.go()
                }).catch(error => {
                    console.log(error)
                });
